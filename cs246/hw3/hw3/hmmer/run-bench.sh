...
do
	...
	do
		...
		sed -i "s|ARGUMENTS|${OUT_FILE} ${CONFIG_NAME} 0 /usr/local/benchmarks/hmmer_O3 /usr/local/benchmarks/inputs/nph3.hmm /usr/local/benchmarks/inputs/swiss41|g" ${SUBMIT_NAME}
...
do
	...
	do
		...
		sed "s|CONFIG|${CONFIG_NAME}|g" $TEMPLATE > $SUBMIT_NAME
		sed -i "s|LOG|${LOG_NAME}|g" $SUBMIT_NAME
		sed -i "s|ARGUMENTS|${OUT_FILE} ${CONFIG_NAME} 1 /usr/local/benchmarks/hmmer_O3 /usr/local/benchmarks/inputs/nph3.hmm /usr/local/benchmarks/inputs/swiss41|g" ${SUBMIT_NAME}
...
do
	...
	do
		TEMPLATE="config.submit-template"
		SUBMIT_NAME="config.submit.${i}.r${j}-victim_l8_direct"
		CONFIG_NAME="config-base-${i}.r${j}-victim_l8_direct"
		LOG_NAME="measure-log-${i}.r${j}-victim_l8_direct"
